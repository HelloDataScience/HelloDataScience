...
이 글은 「생성형 AI로 시작하는 데이터 분석」 9회 연재 중 **9회**입니다. [전체 목차 보기](/genai-series/)
{: .notice--info}
...
| 3단계 | Colab | ipynb 파일을 열어 셀 단위로 실행, 에러 발생 시 Colab 코파일럿으로 수정 | 전처리 결과, 요약 지표, 채워진 보고서 |
...
**3단계**에서 에러가 발생하면 8회차와 똑같이 Colab 코파일럿에게 물어보면 수정된 코드를 받을 수 있습니다. 그런데 한 가지 새로운 상황이 생길 수 있습니다. 보고서 템플릿에 지표가 잘못 입력된 경우인데, 이럴 때는 에러 메시지도 없고 코파일럿에게 보여줄 것도 마땅치 않습니다. 이럴 땐 **보고서 화면을 캡처해서 이미지로 Python Code Assistant에 첨부**하면 됩니다. 그림을 보고 뭐가 잘못됐는지 파악해서 수정된 코드를 알려줍니다.
